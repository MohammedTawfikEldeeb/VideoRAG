...
from pixeltable.functions.openai import chat_completions as openai_chat, embeddings
...
from .functions import extract_caption_from_response, extract_text_from_chunk, resize_image, transcribe_audio
from .tools import re_encode_video
...
os.environ["AUDIO_TRANSCRIPT_MODEL"] = settings.AUDIO_TRANSCRIPT_MODEL
...
class VideoProcessor:
    ...
    def setup_table(self, video_name: str):
        ...
        if exists:
            ...
            cached_table: "TableMetadata" = get_table(self._video_mapping_idx) 
            self.pxt_cache = cached_table.video_cache
            self.video_table = cached_table._video_table_obj   
            self.frames_view = cached_table._frames_view_obj   
            self.audio_chunks = cached_table._audio_chunks_obj 
    ...
    def _check_if_exists(self, video_name: str) -> bool:
        existing_tables = get_registry()
        return video_name in existing_tables
    ...
    def _add_audio_transcription(self):
        self.audio_chunks.add_computed_column(
            transcription=transcribe_audio(self.audio_chunks.audio_chunk),
            if_exists="ignore",
        )
    ...
    def _add_frame_captioning(self):
        messages = [{'role': 'user', 'content': [
            {'type': 'image_url', 'image_url': self.frames_view.resized_frame},
            {'type': 'text', 'text': settings.CAPTION_MODEL_PROMPT},
        ]}]
        self.frames_view.add_computed_column(
            raw_caption_response=openai_chat(messages=messages, model=settings.IMAGE_CAPTION_MODEL),
            if_exists="ignore",
        )
        self.frames_view.add_computed_column(
            im_caption=extract_caption_from_response(self.frames_view.raw_caption_response),
            if_exists="ignore",
        )
